...
{
	if (!m_bgTexture.loadFromFile("resources/state_backgrounds/bg_win_state.png")) throw std::runtime_error("From MenuState - bg image not found");

	sf::Vector2f screenSize(static_cast <sf::Vector2f> (m_window.getSize()));
	m_backGround = Background(screenSize, m_bgTexture);
	m_startButton = Button("TO LOADING STATE", sf::Vector2f(screenSize.x / 4, screenSize.y / 4), sf::Vector2f(screenSize.x / 2, screenSize.y / 2), sf::Color::Transparent, 30);
